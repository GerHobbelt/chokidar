...
function setFSEventsListener(path, realPath, listener, rawEmitter) {
  ...
  function filteredListener(fullPath_, flags, info) {
    var fullPath = fullPath_;
...
function depth(path_, root) {
  var path = path_;
  var i = 0;
  while (path.indexOf(root) === 0 && sysPath.dirname(path) !== root) {
      ...
      fs.open(path, 'r', function(error, fd) {
        ...
        if (error) {
          ...
          fs.close(fd, function(err) {
            if (err && err.code !== 'EACCES') handleEvent('unlink');
            else addOrChange();
          });
        }
      });
  ...
  fs[wh.statMethod](wh.watchPath, function(error, stats) {
    ...
    if (stats.isDirectory()) {
      ...
      readdirp(wh.watchPath, {
        ...
        depth: typeof this.options.depth === 'number' ? this.options.depth - (priorDepth || 0) : Number.MAX_SAFE_INTEGER,
        type: 'all',
        alwaysStat: true,
        lstat: true
